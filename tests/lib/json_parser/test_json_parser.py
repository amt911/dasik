...
def test_invalid_subsection_exits_with_code_2(tmp_path):
    # Top-level sections are all optional now, but a PRESENT section with a
    # missing required inner field is still invalid → exit 2 (LocaleModel needs
...
def test_missing_optional_sections_still_parse(tmp_path):
    cfg = _valid()
    for k in ("network", "locales", "timezone", "hostname"):
        cfg.pop(k, None)                             # all optional now
    jp = JsonParser(_write(tmp_path, cfg))           # must not exit
    assert jp.get_attr("network") is None
    assert jp.get_attr("locales") is None
    assert jp.get_attr("hostname") == ""
