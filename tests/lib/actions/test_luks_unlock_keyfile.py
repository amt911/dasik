...
def test_formatting_a_disk_no_longer_enrolls_the_keyfile():
    """Enrollment moved to LuksKeyfileAction, which owns it for fresh installs
    AND for a machine that gains a pendrive later (see
    tests/lib/actions/test_luks_keyfile_action.py). Two owners would add a
    redundant keyslot on every format."""
    assert not hasattr(DiskPartitionAction, "_add_unlock_keyfile")
